...
// File: @unlock-protocol/contracts/dist/PublicLock/IPublicLockV12.sol
...
// File: contracts/Hooks.sol

//SPDX-License-Identifier: Unlicense


// whitelist users [array]
// remove from whitelist [array]
// setpremintEndTime :  in respect to time duration
// SetPremintPrice
// Purchase : validation for price
...
contract PurchaseHook {
    ...
    uint256 public preMintEndTime = block.timestamp + 2 * 1 days; // Make it how many ever days are neded
    uint256 public numberOfWhitelistedAddresses = 0;
    IPublicLockV12 public lock = IPublicLockV12(0x719D96b6338E2a0db895d8781318A519EbC3e9c1);

    event WhiteListAddressAdded(address indexed _address);
    event WhiteListAddressRemoved(address indexed _address);
    event PreMintTimeChanged(uint256 newEndTime);
    event PreMintPriceChange(uint256 newPreMintPrice);

    function addToWhiteList(address _address, address _publicLockAddress) external onlyLockManager(_publicLockAddress) {
    ...
    function removeFromWhiteList(address _address, address _publicLockAddress ) external onlyLockManager(_publicLockAddress)  {
        // Should there be a modifier for authorised calling of this function?
        require(
            isWhiteListed(_address),
            "Sorry, the address is not whitlisted"
        );
        numberOfWhitelistedAddresses--;

        whiteList[_address] = false;
    }
    function stopMinting (address _publicLockAddress) external onlyLockManager(_publicLockAddress) {
  
    uint256 numberOfKeysAlreadySold = IPublicLockV12(_publicLockAddress).totalSupply();  
    // lock.isLockManager(account);
    IPublicLockV12(_publicLockAddress).updateLockConfig(1, numberOfKeysAlreadySold+1,1 );  


    }
  
    function startMinting (address _publicLockAddress, uint256 duration, uint256 numberOfKeys, uint256 numberOfKeysPerWallet , address tokenAddress_) external onlyLockManager(_publicLockAddress){
    uint256 numberOfKeysAlreadySold = IPublicLockV12(_publicLockAddress).totalSupply();  
    // bool isLockManager =  IPublicLockV12(_publicLockAddress).isLockManager(msg.sender);  
    // require(isLockManager,"Sorry you need to be lockManager to access the function");
    require(numberOfKeys>=numberOfKeysAlreadySold, "Sorry, you should have the number of keys to be >= number of keys already sold");
    uint256 durationInSeconds = duration*86400;
    IPublicLockV12(_publicLockAddress).updateLockConfig(durationInSeconds, numberOfKeys, numberOfKeysPerWallet);  
    IPublicLockV12(_publicLockAddress).updateKeyPricing(IPublicLockV12(_publicLockAddress).keyPrice(), tokenAddress_);  

   
    }

    function getCountOfWhiteList() external view returns (uint256) {
    ...
    function changePreMintTime(uint256 _durationInDays, address _publicLockAddress) external onlyLockManager(_publicLockAddress) {

        preMintEndTime = block.timestamp + _durationInDays * 1 days;
        emit PreMintTimeChanged(preMintEndTime);
    }

    function changePreMintPrice(uint256 _price, address _publicLockAddress) external  onlyLockManager(_publicLockAddress) {
        preMintPrice = _price * 1 ether;
        emit PreMintPriceChange(preMintPrice);
    }

    modifier onlyLockManager(address _publicLockAddress) {
        bool isLockManager =  IPublicLockV12(_publicLockAddress).isLockManager(msg.sender);  

        require(
            isLockManager,
            "Only the LockManager can call this function"
        );
        _;
    }
    ...
    {
        if (block.timestamp >= preMintEndTime) {
            return IPublicLockV12(msg.sender).keyPrice();
        }
        else if ((block.timestamp < preMintEndTime) && (isWhiteListed(recipient))){
            return preMintPrice;
        }
